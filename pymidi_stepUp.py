...
def main():
    ...
    # This loop steps up the musical scale.
    for i in range(n_sounds):
        j = i + 35
        time.sleep(0.1)
        midi_out.note_on(note= j,velocity=127)
        time.sleep(0.3)
        midi_out.note_off(note= j,velocity=0)

        frequencyOfnote = pygame.midi.midi_to_frequency(j)
        print(f"Instrument ", instrument, "Note ", j, "has the frequency ", frequencyOfnote)
